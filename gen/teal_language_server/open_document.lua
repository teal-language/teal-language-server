local _tl_compat; if (tonumber((_VERSION or ''):match('[%d.]*$')) or 0) < 5.3 then local p, m = pcall(require, 'compat53.module'); if p then _tl_compat = m end end; local ipairs = _tl_compat and _tl_compat.ipairs or ipairs; local pairs = _tl_compat and _tl_compat.pairs or pairs; local string = _tl_compat and _tl_compat.string or string; local table = _tl_compat and _tl_compat.table or table; local _module_name = "open_document"
...
function OpenDocument:__init(uri, server_state, module_info)
...
function OpenDocument:_try_get_type_report(env)
...
function OpenDocument:_get_ast_node_at(type_info, env)
...
function OpenDocument:get_function_args_string(type_info, env)
   local node = self:_get_ast_node_at(type_info, env)
...
function OpenDocument:resolve_type_ref(type_number, env)
...
function OpenDocument:type_information_for_tokens(tokens, y, x, env)
   local tr = self:_try_get_type_report(env)
